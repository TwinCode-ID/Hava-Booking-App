...
exports.createInstructor = async (req, res) => {
    try {
        const {fullName,bio,assignedStudiosId,avatar,workingHours,instructorType} = req.body;
        if (!fullName){
            return res.status(400).json({message: "Instructor name is required"});
        }
        const instructor = await Instructor.create({fullName,bio,assignedStudiosId,avatar,workingHours,instructorType});
...
exports.updateProfile = async (req, res) => {
    try {
        const {fullName,bio,assignedStudiosId,avatar,workingHours,instructorType, isActive} = req.body;
        ...
        instructor.assignedStudiosId = assignedStudiosId || instructor.assignedStudiosId;
