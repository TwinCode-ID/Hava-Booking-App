...
exports.createPackage = async (req, res) => {
    try {
        const {packageName,packageDescription,packagePrice,currency,validityDays,instructorType} = req.body;
        if (!packageName){
            return res.status(400).json({message: "Package name is required"});
        }
        const studioLocation = req.user.adminStudioLocation;
        const package = await Package.create({packageName,packageDescription,packagePrice,currency,validityDays,instructorType,studioLocation});
...
exports.updatePackage = async (req, res) => {
    try {
        const {packageName,packageDescription,packagePrice,currency,validityDays,isActive,instructorType} = req.body;
        const package = await Package.findById(req.params.id);
        if(!package){
            return res.status(404).json({message: "Package not found"});
        }
...
exports.packageStatus = async (req, res) => {
    try {
        ...
        if (package.studioLocation.toString() !== req.user.adminStudioLocation.toString()){
