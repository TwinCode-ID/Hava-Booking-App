...
const BookingModal = ({ cls, onClose, onConfirm }) => {
  ...
  // 1. Fetch User's Active Passes when modal opens
  useEffect(() => {
    const fetchPasses = async () => {
      try {
        // Adjust this URL to match your "getMyActivePasses" route
        // Assuming route is: /api/user-passes/my-active-passes
        ...
        // Auto-select the first pass if available
        if (res.data.length > 0) {
          setSelectedPassId(res.data[0]._id);
        }
      } catch (err) {
  ...
  const handleBook = async () => {
    // 2. Validate that a pass is selected
    if (!selectedPassId) {
    ...
    try {
      // 3. Send both classId AND passId
      await axiosInstance.post(API_PATHS.BOOKING.CREATE_BOOKING, {
        classId: cls._id, // Backend expects 'classId'
        passId: selectedPassId, // Backend expects 'passId'
      });

      onConfirm();
    } catch (err) {
      setError(
        err.response?.data?.error || // Check for 'error' field from backend
