    ...
    <>
      ...
      <div className='fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200'>
        ...
        <div className='bg-white w-full max-w-lg rounded-t-[2rem] md:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] md:max-h-[85vh] animate-in slide-in-from-bottom-8 md:slide-in-from-bottom-0'>
          ...
          <div className='p-4 md:p-6 overflow-y-auto flex-1 custom-scrollbar overscroll-contain'>
            {fetchingPasses ? (
              <div className='text-center py-10 flex flex-col items-center'>
                <div className='w-8 h-8 border-4 border-stone-500 border-t-transparent rounded-full animate-spin mb-3'></div>
              ...
              <div className='space-y-5 md:space-y-6'>
                {classes.map((cls) => {
                  ...
                  return (
                    <div
                      key={cls._id}
                      className={`rounded-2xl border transition-all ${assignedPass ? "border-stone-200" : "border-red-200 bg-red-50/30"}`}>
                      {/* Class Details */}
                      <div className='p-4 border-b border-stone-100/50 bg-stone-50/50 rounded-t-2xl flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3'>
                        <div>
                        ...
                        <div className='flex flex-row sm:flex-col items-center sm:items-end gap-2'>
                          <span className='text-[11px] md:text-xs font-bold px-2 py-1 rounded-md bg-white border border-stone-200 text-stone-600 uppercase tracking-wider'>
                            {cls.classType}
                          </span>
                          <span className='text-[11px] md:text-xs font-bold px-2 py-1 rounded-md bg-stone-100 border border-stone-200 text-stone-800 uppercase tracking-wider'>
                            {cls.instructorType}
                          </span>
                        </div>
                      </div>
                          ...
                          <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-stone-100 border border-stone-200 p-3.5 md:p-3 rounded-xl'>
                            <div className='flex items-center gap-3'>
                              ...
                              <div>
                                ...
                                <p className='text-xs md:text-sm text-stone-800 mt-0.5'>
                                  Exp:{" "}
                                  {format(
                            ...
                            <button
                              ...
                              className='text-sm md:text-xs font-bold text-stone-800 hover:text-stone-900 flex justify-center items-center gap-1 bg-white px-3 py-2 md:px-2 md:py-1 rounded-lg border border-stone-300 transition-colors w-full sm:w-auto'>
                          ...
                          <div className='space-y-3 animate-in slide-in-from-top-2'>
                            <div className='flex items-center justify-between mb-2'>
                              ...
                              <button
                                ...
                                className='text-sm md:text-xs text-stone-400 hover:text-stone-600 py-1 px-2'>
                                Cancel
                              </button>
                            </div>
                            {eligiblePasses.map((pass) => {
                              ...
                              return (
                                <label
                                  key={pass._id}
                                  className={`flex items-center justify-between p-4 md:p-3 rounded-xl border cursor-pointer transition-all ${assignedPassId === pass._id ? "border-stone-500 bg-stone-100 ring-1 ring-stone-500" : isSelectable ? "border-stone-200 hover:bg-stone-50" : "border-stone-100 bg-stone-50 opacity-50"}`}>
                                  <div className='flex items-center gap-4 md:gap-3'>
                                    <input
                                      type='radio'
                                      name={`pass-${cls._id}`}
                                      className='accent-stone-600 w-5 h-5 md:w-4 md:h-4 shrink-0'
                                  ...
                                  <div className='text-right shrink-0'>
                                    <p
                                      className={`text-base md:text-sm font-bold ${availableCredits > 0 ? "text-stone-800" : "text-red-500"}`}>
                                      {availableCredits}
                                    </p>
                            ...
                            {eligiblePasses.length === 0 && (
                              <p className='text-sm text-stone-500 italic p-4 text-center bg-stone-50 rounded-xl'>
          ...
          {/* Footer Actions */}
          <div className='p-4 md:p-6 border-t border-stone-100 bg-white shrink-0 pb-safe md:pb-6'>
            {!fetchingPasses &&
              (isCartFullyCovered ? (
                <button
                  onClick={handleBook}
