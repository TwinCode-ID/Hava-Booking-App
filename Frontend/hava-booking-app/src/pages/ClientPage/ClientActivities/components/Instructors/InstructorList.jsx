...
const InstructorList = () => {
    ...
    <div className='space-y-8'>
      ...
      <div className='space-y-4'>
        <h3 className='text-sm font-bold text-stone-900 uppercase tracking-wider flex items-center gap-2'>
          <Star className='w-4 h-4 text-stone-400' /> Level
        </h3>
      ...
      <div className='space-y-4'>
        <h3 className='text-sm font-bold text-stone-900 uppercase tracking-wider flex items-center gap-2'>
          <MapPin className='w-4 h-4 text-stone-400' /> Studio Location
        </h3>
        <div className='space-y-3 md:space-y-2'>
      ...
      <div className='space-y-4'>
        <div className='flex items-center justify-between'>
          <h3 className='text-sm font-bold text-stone-900 uppercase tracking-wider flex items-center gap-2'>
            <Calendar className='w-4 h-4 text-stone-400' /> Available On
          </h3>
        </div>
  ...
  if (loading)
    return (
      <div className='min-h-[60vh] flex flex-col items-center justify-center'>
        <LoadingSpinner />
        <p className='text-stone-500 font-medium mt-4 text-sm'>
          Loading instructors...
        </p>
      </div>
    );
  ...
  return (
    <div className='min-h-screen bg-stone-50/50 font-sans text-stone-900 flex flex-col'>
      <div className='container mx-auto px-4 md:px-6 py-8 md:py-12 max-w-7xl'>
        <div className='flex flex-col lg:flex-row gap-8 xl:gap-12'>
              ...
              <div className='flex items-center justify-between mb-6'>
                ...
                {getActiveFilterCount() > 0 && (
                  <button
                    ...
                    className='text-xs font-bold text-stone-800 hover:text-stone-900'>
                    Clear All
                  </button>
                )}
              </div>
          ...
          <div className='flex-1 min-w-0'>
            {/* Header Area: Search */}
            <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-8 pb-4 md:pb-6 border-b border-stone-200/60'>
              <div>
              ...
              <div className='flex gap-3 w-full sm:w-auto'>
                <div className='relative flex-1 sm:w-64 md:w-72'>
                  <Search className='absolute left-3.5 top-1/2 -translate-y-1/2 w-[18px] h-[18px] text-stone-400' />
                  <input
                    type='text'
                    placeholder='Search instructor...'
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className='w-full pl-10 pr-4 py-3 md:py-2.5 bg-white border border-stone-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-stone-500/20 focus:border-stone-500 transition-all shadow-sm'
                  />
                </div>
                <button
                  className='lg:hidden flex items-center justify-center gap-2 text-sm font-bold text-stone-900 bg-white border border-stone-200 px-4 py-3 rounded-xl hover:bg-stone-50 transition-colors shadow-sm relative shrink-0'
              ...
              {filteredInstructors.length > 0 ? (
                ...
                <div className='col-span-full py-20 text-center bg-white rounded-3xl border border-dashed border-stone-200'>
                  ...
                  {getActiveFilterCount() > 0 && (
                    <button
                      onClick={clearAllFilters}
                      className='mt-6 px-6 py-2.5 bg-stone-100 text-stone-800 font-bold rounded-xl text-sm hover:bg-stone-200 transition-colors'>
                      Clear Filters
                    </button>
                  )}
                </div>
        ...
        {showMobileFilters && (
          <div className='fixed inset-0 z-50 flex items-end justify-center lg:hidden p-0 bg-black/60 backdrop-blur-sm'>
            ...
            <motion.div
              ...
              {/* Sticky Footer */}
              <div className='p-4 md:p-6 border-t border-stone-100 bg-white shrink-0 flex gap-3'>
                <button
                  onClick={clearAllFilters}
                  disabled={getActiveFilterCount() === 0}
                  className='px-6 py-3.5 bg-stone-100 text-stone-700 font-bold rounded-xl text-sm hover:bg-stone-200 transition-colors disabled:opacity-50 flex-1 md:flex-none'>
                  Reset
                </button>
                <button
                  onClick={() => setShowMobileFilters(false)}
    ...
    <div
      ...
      <div className='flex items-start justify-between mb-5 md:mb-6'>
        ...
        <div className='text-right ml-3'>
          <span className='inline-block bg-stone-100 text-stone-900 text-[9px] md:text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-md border border-stone-200'>
            {data.instructorType}
          </span>
        </div>
      </div>
      ...
      <div className='flex-1 mb-5 md:mb-6'>
        <h3 className='font-bold text-stone-900 text-lg md:text-xl mb-1.5 md:mb-2 group-hover:text-stone-800 transition-colors line-clamp-1'>
          {data.fullName}
        </h3>

        {/* Short Bio Snippet */}
        <p className='text-stone-500 text-[13px] md:text-sm line-clamp-2 h-10'>
          {data.bio ||
        ...
        <div className='grid grid-cols-2 gap-3 md:gap-4 text-sm mt-5 md:mt-6 pt-2'>
          <div className='flex items-center gap-1.5 md:gap-2 text-stone-700'>
            <MapPin className='w-[14px] h-[14px] md:w-4 md:h-4 text-stone-400 shrink-0' />
          ...
          <div className='flex items-center gap-1.5 md:gap-2 text-stone-700'>
            <CalendarDays className='w-[14px] h-[14px] md:w-4 md:h-4 text-stone-400 shrink-0' />
            <span className='font-medium text-[11px] md:text-xs truncate'>
              {activeDays.length > 0
  ...
  return (
    <div className='fixed inset-0 z-[60] flex items-end md:items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-sm'>
      ...
      <motion.div
        ...
        {/* Header */}
        <div className='flex justify-between items-center px-6 py-4 md:p-6 border-b border-stone-100 bg-white shrink-0 z-10'>
          <div>
        ...
        <div className='p-6 md:p-8 overflow-y-auto flex-1 custom-scrollbar overscroll-contain pb-safe'>
          ...
          <div className='flex flex-col items-center text-center mb-8'>
            <div className='w-20 h-20 md:w-24 md:h-24 rounded-full p-1 bg-gradient-to-br from-stone-300 to-stone-100 mb-4 md:mb-5 shadow-lg shrink-0'>
              <img
                src={
            ...
            <div className='mt-2'>
              <span className='px-3 py-1 bg-stone-100 text-stone-800 text-[10px] md:text-xs font-bold uppercase tracking-wider rounded-md border border-stone-200'>
          ...
          <div className='space-y-6 md:space-y-8'>
            {/* Bio */}
            <div>
              <h4 className='text-[10px] md:text-[11px] font-bold text-stone-400 uppercase tracking-wider mb-2 md:mb-3 flex items-center gap-2'>
                Biography
              </h4>
              <div className='bg-stone-50/80 p-4 md:p-5 rounded-2xl border border-stone-100'>
                <p className='text-stone-600 text-[13px] md:text-sm leading-relaxed'>
                  {instructor.bio ||
              ...
              <div className='grid grid-cols-1 gap-2 md:gap-3'>
                {instructor.assignedStudiosId.length > 0 ? (
                  instructor.assignedStudiosId.map((studio) => (
                    <div
                      key={studio._id}
                      className='px-4 py-3.5 bg-white border border-stone-200 rounded-xl text-sm text-stone-700 shadow-sm flex items-center gap-3'>
                      <div className='p-2 bg-stone-100 rounded-full text-stone-800 shrink-0'>
                ...
                {hasSchedule ? (
                  DAYS_ORDER.map((day) => {
                    ...
                    return (
                      <div
                        key={day}
                        className='flex flex-col sm:flex-row sm:items-start p-4 border-b border-stone-100 last:border-0 hover:bg-stone-50 transition-colors'>
                        <div className='sm:w-28 shrink-0 mb-3 sm:mb-0'>
                          <span className='text-[13px] md:text-sm font-bold text-stone-900 capitalize block mt-0.5'>
                            {day}
                          </span>
                        </div>
                        <div className='flex flex-wrap gap-2 flex-1'>
                          {timeSlots.map((slot, index) => (
                            <span
                              key={slot._id || index}
                              className='px-2.5 py-1.5 bg-stone-100 text-stone-800 text-[11px] md:text-xs font-bold rounded-lg border border-stone-200'>
                              {slot.start} - {slot.end}
                            </span>
                          ))}
                        </div>
